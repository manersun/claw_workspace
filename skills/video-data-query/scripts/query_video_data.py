...
Usage:
    python query_video_data.py --stream-id "eXkmWrpRCdU" [options]
"""
...
# Cookie 配置
DEFAULT_COOKIE_FILE = Path.home() / ".openclaw" / "video-data-cookie.txt"


def load_cookie(cookie_file: Optional[str] = None) -> str:
    """加载 Cookie"""
    # 优先级：命令行参数 > 环境变量 > 配置文件
    if os.getenv("VIDEO_DATA_COOKIE"):
        return os.getenv("VIDEO_DATA_COOKIE")
    ...
    raise ValueError(
        "Cookie not found. Please set VIDEO_DATA_COOKIE environment variable "
        f"or create cookie file at {cookie_path}"
    )
    ...
    if "data" in data and isinstance(data["data"], list):
        for i, row in enumerate(data["data"][:max_rows]):
            if isinstance(row, dict):
                for key, value in list(row.items())[:3]:  # 每行显示前 3 个字段
...
def main():
    parser = argparse.ArgumentParser(
        description="快手视频数据查询工具",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  python query_video_data.py --stream-id "eXkmWrpRCdU"
  python query_video_data.py --stream-id "xxx" --start-time 1772013000000 --end-time 1772013600000
  python query_video_data.py --stream-id "xxx" --output json
        """
    )
    ...
    try:
        # 获取 Cookie
        if args.cookie_content:
            cookie = args.cookie_content
        else:
        ...
        # 保存日志
        log_file = Path.home() / ".openclaw" / "video-data-query.log"
