...
# Cookie 配置
DEFAULT_COOKIE_FILE = Path.home() / ".openclaw" / "arya-log-cookie.txt"


def load_cookie(cookie_file: Optional[str] = None) -> str:
    """加载 Cookie"""
    # 优先级：命令行参数 > 环境变量 > 配置文件
    if os.getenv("ARYA_LOG_COOKIE"):
        return os.getenv("ARYA_LOG_COOKIE")
    ...
    raise ValueError(
        "Cookie not found. Please set ARYA_LOG_COOKIE environment variable "
